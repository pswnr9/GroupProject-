...
CREATE TABLE `financial_access` (
  `form_id` int REFERENCES form_info(form_id),
  PRIMARY KEY (`form_id`),
  `general_inquiry` tinyint(1) DEFAULT NULL,
  `cash_group_post` tinyint(1) DEFAULT NULL
);
...
CREATE TABLE `student_record_access` (
  ...
  `department_SOC_update` tinyint(1) DEFAULT NULL,
  `service_indicators` tinyint(1) DEFAULT NULL,
  `student_group_view` tinyint(1) DEFAULT NULL,
